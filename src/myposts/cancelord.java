...
public class cancelord extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        addToCart.setText("CANCEL ORDER");
    ...
    private void addToCartActionPerformed(java.awt.event.ActionEvent evt) {                                          
         addToCart.setBackground(new Color(0,153,255));      
            String brnd=this.productInfoBrandName.getText();
            String pname=this.productInfoModel.getText();
            String pstuid="";
            ...
            try {
                        ...
                        Statement st = conn.createStatement();
                          ...
                          ResultSet insval=st.executeQuery("select * from producttable where user_id='"+us1+"' and p_name='"+pname+"' AND brand='"+brnd+"' AND status='yes'");
                          while(insval.next())
                          {
                           pstuid=insval.getString("post_id");
                          }
                          System.out.println("postid:"+pstuid);
                          
                          st.executeUpdate("update displaytable set status='approved' where user_id='"+pstuid+"' AND p_name='"+pname+"' AND brand='"+brnd+"' AND status='ordered'");
                          st.executeUpdate("update producttable set status='approved' where user_id='"+pstuid+"' AND p_name='"+pname+"' AND brand='"+brnd+"' AND status='ordered'");
                          st.executeUpdate("delete from producttable where user_id='"+us1+"' and p_name='"+pname+"' AND brand='"+brnd+"' AND status='yes'");
